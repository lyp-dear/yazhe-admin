<template>
	<div>
		<el-table :data="tableData" border style="width: 100%">
			...
			<el-table-column prop="value" label="名称" width="180" align="center">
			</el-table-column>
			<el-table-column prop="sizeConfigName" label="尺寸类型" width="180" align="center">
			</el-table-column>
...
<script>
	import { SizeTypeValueList, UpdateSizeTypeValue } from 'api/sizeTypeValue'
	...
	export default {
		...
		methods: {
			...
			removeClick(row) {
				this.$confirm('此操作将永久删除尺寸类型值, 是否继续?', '提示', {
					...
					UpdateSizeTypeValue(options).then((res) => {
						...
						if(res.data.code == ERR_CODE) {
							showNotification('success', '删除尺寸类型值成功');
							this.getSizeTypeValueList();
						} else {
			...
			getSizeTypeValueList() {
				const loading = showLoading();
				SizeTypeValueList(this.currentPage).then((res) => {
					console.log(res)
					loading.close();
					if(res.data.code == ERR_CODE) {
						this.tableData = [];
						...
						for(let i = 0; i < data.length; i++) {
							...
							let obj = {
								...
								'value': data[i].value,
								'image': data[i].image,
								'sizeConfigName': data[i].sizeConfigName
