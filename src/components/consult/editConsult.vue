		...
		<div class="container">
			...
			<el-form :model="data" :rules="rules" ref="data" label-width="100px" class="demo-ruleForm" :label-position="labelPosition">
				...
				<el-form-item label="封面" prop="picture">
					<upload-image v-on:selectCover="getCover" :imgUrl="data.picture"></upload-image>
				</el-form-item>
				...
				<el-form-item label="内容" prop="context">
					<editor v-on:listenToDetail="getDetail" :description="data.context"></editor>
				</el-form-item>
				<el-form-item>
...
<script>
	...
	import { AddConsult, UpdateConsult } from 'api/consult'
			...
			submitForm(formName) {
				this.$refs[formName].validate((valid) => {
					if(valid) {
						...
						options.status = 0;
						options.newsId = this.$route.params.newsId;
						if(this.data.type === '着装品格') {
							options.type = 1;
						}
						if(this.data.type === '面料养护') {
							options.type = 2;
						}
						if(this.data.type === '奢侈驿站') {
							options.type = 3;
						}
						if(this.data.type === '财经咨询') {
							options.type = 4;
						}
						if(this.data.type === '公司咨询') {
							options.type = 5;
						}
						UpdateConsult(options).then((res) => {
							loading.close();
