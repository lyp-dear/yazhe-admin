<template>
	<div>
		<el-table :data="tableData" border style="width: 100%">
			...
			<el-table-column prop="title" label="类型" width="180" align="center">
			</el-table-column>
...
<script>
	import { SizeTypeList, UpdateSizeType } from 'api/sizeType'
	...
	export default {
		...
		methods: {
				...
				this.$confirm('此操作将永久删除尺寸类型, 是否继续?', '提示', {
					...
					UpdateSizeType(options).then((res) => {
						...
						if(res.data.code == ERR_CODE) {
							showNotification('success', '删除尺寸类型成功');
			...
			getSizeTypeList() {
				const loading = showLoading();
				SizeTypeList(this.currentPage).then((res) => {
					console.log(res)
					loading.close();
					if(res.data.code == ERR_CODE) {
						this.tableData = [];
